...
export const UserContextProvider = ({ children }) => {
  ...
  const registerUser = async (e) => {
    ...
    try {
      ...
      console.log("Error registrando un usuario", error);
  ...
  const updateProfilePicture = async (formData) => {
    setLoading(true);
    try {
      const res = await axios.patch(`${serverUrl}/api/v1/user/profile-picture`, formData, {
        headers: { "Content-Type": "multipart/form-data" }, // Important for sending files
        withCredentials: true,
      });
      setUser((prevState) => ({ ...prevState, photo: res.data.photo })); // Update only the photo
  ...
  return (
    <UserContext.Provider
      value={{
        ...
        allUsers,
        deleteUser,
        updateProfilePicture, // Asegúrate de incluir la nueva función aquí
        loading, // También puedes exponer el estado de carga si lo necesitas en el modal
      }}
    >
      {children}
    </UserContext.Provider>
