...
function ProfileModal() {
  ...
  const { user, updateUser, handlerUserInput, userState, changePassword, updateProfilePicture } =
    useUserContext();
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    let profilePictureUpdated = true;
    if (newProfilePicture) {
      const profileFormData = new FormData();
      profileFormData.append("profileImage", newProfilePicture);
      profilePictureUpdated = await updateProfilePicture(profileFormData);
      if (!profilePictureUpdated) {
        setUploadError("Error al actualizar la foto de perfil.");
        setUploading(false);
        return;
      }
    }
    ...
    const userDetailsUpdated = await updateUser(e, userData);

    if (userDetailsUpdated && profilePictureUpdated) {
      closeModal();
    } else if (!userDetailsUpdated && !uploadError) {
      setUploadError("Error al actualizar los detalles del perfil.");
    }
      ...
      >
        ...
        <div className="mt-4 relative flex justify-between">
          <div className="relative inline-block">
            ...
            />
            <div className="absolute bottom-0 right-1 shadow-sm cursor-pointer" onClick={handleUploadClick}>
